...
<c:if test="${not empty errorMessage}">
  <div style="background:#ffe5e5;color:#9b1c1c;padding:12px 16px;border-radius:6px;margin:16px 0;font-weight:700;">
      ${errorMessage}
  </div>
</c:if>
            ...
            <tbody id="dashTbody">
              <c:forEach var="p" items="${products}">
                <tr
                  ...
                  data-category-name="<c:out value='${p.categoryName}'/>"
                  data-category-id="${p.categoryId}"
                  data-price="${p.price}"
                  data-oldprice="${empty p.oldPrice ? 0 : p.oldPrice}"
                  data-rating="${p.rating}"
                  data-in-stock="${p.inStock}"
                  data-stock-quantity="${p.stockQuantity}"
                  ...
                  <td>
                    <div style="display:flex;align-items:center;gap:.75rem;">
                      ...
                      <div>
                        ...
                        <div class="td-desc"><c:out value="${p.description}" /></div>
                      </div>
                    </div>
                  </td>
            ...
            <tbody id="prodMgmtTbody">
              <c:forEach var="p" items="${products}">
                ...
                >
                  ...
                  <td>
                    <c:choose>
                      <c:when test="${not empty p.oldPrice}">
                        Rs <fmt:formatNumber value="${p.oldPrice}" pattern="#,##0" />
                      </c:when>
    ...
    <div id="sec-sales" style="display:none">
      ...
      <div class="card">
        <div class="card-head">
          ...
          <select id="salesChartRange" class="chart-range-select">
            <option value="7">Last 7 days</option>
            <option value="14">Last 14 days</option>
      ...
      <form id="productForm" action="${pageContext.request.contextPath}/admin" method="post" enctype="multipart/form-data">
        ...
        <div class="fg-row">
          <div class="fg">
            ...
            <input type="text" id="pName" name="name" placeholder="e.g. Casio Edifice EF-539D" />
          </div>
          <div class="fg">
            <label for="pBrand">Brand *</label>
            <input type="text" id="pBrand" name="brand" placeholder="e.g. Casio" />
          </div>
        </div>
        <div class="fg-row">
          <div class="fg">
            <label for="pPrice">Price (Rs) *</label>
            <input type="number" id="pPrice" name="price" placeholder="12500" min="0" />
          </div>
          <div class="fg">
            <label for="pOldPrice">Original Price (Rs)</label>
            <input type="number" id="pOldPrice" name="oldPrice" placeholder="14000 — optional" min="0" />
          </div>
        </div>
        <div class="fg-row">
          <div class="fg">
          ...
          <div class="fg">
            ...
            <input type="number" id="pRating" name="rating" placeholder="4.5" min="0" max="5" step="0.1" />
          </div>
        </div>
        <div class="fg">
          <label for="pDesc">Description</label>
          <textarea id="pDesc" name="description" placeholder="Brief product description…"></textarea>
        </div>
        <div class="fg">
          <label for="pImageFile">Product Image</label>
          <input type="file" id="pImageFile" name="imageFile" accept="image/*" />
          <div class="field-note">Choose an image file, or enter a URL below.</div>
        </div>
        <div class="fg">
          <label for="pImg">Image URL</label>
          <input type="text" id="pImg" name="imageUrl" placeholder="Optional image URL" />
        </div>
        <div class="fg">
          <div class="toggle-wrap">
            <label class="toggle" for="pStock">
              <input type="checkbox" id="pStock" name="inStock" checked />
              <div class="fg">
...
function showSection(id, el) {
  var ids = ['dashboard','products','orders','customers','sales','inventory'];
  for (var i = 0; i < ids.length; i++) {
    var section = document.getElementById('sec-' + ids[i]);
    if (section) {
      section.style.display = ids[i] === id ? 'block' : 'none';
    }
  }
  var buttons = document.querySelectorAll('.adm-nav-item');
  ...
  var titles = {
    ...
    sales:'Sales Report', inventory:'Inventory'
  };
...
function getDataRows(bodyId) {
  var rows = document.querySelectorAll('#' + bodyId + ' tr[data-id]');
  var list = [];
...
function updateDashboardCounts() {
  ...
  var inStock = 0;
  for (var i = 0; i < rows.length; i++) {
      if (parseInt(rows[i].dataset.stockQuantity || '0') > 0) { inStock++; }
  }
  document.getElementById('dashProductCount').textContent = count + ' product' + (count !== 1 ? 's' : '') + ' in catalogue';
  document.getElementById('kpiStockCount').textContent = inStock;
}
...
function updateProductCounts() {
  ...
  document.getElementById('prodMgmtCount').textContent = total + ' products';
  document.getElementById('pStatTotal').textContent = total;
  document.getElementById('pStatInStock').textContent = inStock;
  document.getElementById('pStatOut').textContent = total - inStock;
}

function filterDashTable() {
  var query = document.getElementById('dashSearch').value.toLowerCase();
  var rows = getDataRows('dashTbody');
  for (var i = 0; i < rows.length; i++) {
    var row = rows[i];
    var name = row.dataset.name.toLowerCase();
    var brand = row.dataset.brand.toLowerCase();
    var category = row.dataset.categoryName.toLowerCase();
    var show = !query || name.indexOf(query) !== -1 || brand.indexOf(query) !== -1 || category.indexOf(query) !== -1;
    row.style.display = show ? '' : 'none';
  }
}

function filterProdTable() {
  var query = document.getElementById('prodSearch').value.toLowerCase();
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    var name = row.dataset.name.toLowerCase();
    var brand = row.dataset.brand.toLowerCase();
    var category = row.dataset.categoryName;
    var textMatch = !query || name.indexOf(query) !== -1 || brand.indexOf(query) !== -1;
    var catMatch = !cat || category === cat;
...
function openEditModal(row) {
  ...
  document.getElementById('pStock').checked = row.dataset.inStock === 'true';
...
function saveProduct() {
  ...
  var category = form.categoryId.value;
...
document.getElementById('delModal').addEventListener('click', function(event) {
  if (event.target === event.currentTarget) { closeDelModal(); }
});
